    ...
    private void initNormal() {
        ...
        ReflectionUtils.doWithLocalFields(DefaultKafkaConsumerFactory.class, field -> {
            if (field.getName().equals("configs")) {
                ...
                configs.put(ConsumerConfig.PARTITION_ASSIGNMENT_STRATEGY_CONFIG, StickyAssignor.class.getName());
                // 不是用自动提交
                ...
                // 单次获取的最大消息数，使用默认的值
                configs.put(ConsumerConfig.FETCH_MAX_BYTES_CONFIG, ConsumerConfig.DEFAULT_FETCH_MAX_BYTES);
                ...
                configs.put(ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG, ConsumerConfig.DEFAULT_MAX_PARTITION_FETCH_BYTES);
                // tcp缓存，使用默认值
                configs.put(ConsumerConfig.SEND_BUFFER_CONFIG, 128 * 1024);
                // tcp缓存，使用默认值
                configs.put(ConsumerConfig.RECEIVE_BUFFER_CONFIG, 64 * 1024);
                // 重试时间，默认100ms
                configs.put(ConsumerConfig.RETRY_BACKOFF_MS_CONFIG, 100L);
